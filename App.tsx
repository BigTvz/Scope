...
const Dashboard: React.FC = () => {
  ...
  const [expenses, setExpenses] = useState<Expense[]>(() => {
    return StorageService.getItem(user.id, 'expenses', []);
  });

  const [settings, setSettings] = useState<UserSettings>(() => {
    return StorageService.getItem(user.id, 'settings', { exchangeRate: 1.0, localCurrencySymbol: 'USD' });
  });

  const [rates, setRates] = useState<ExchangeRates>(() => {
    return StorageService.getItem(user.id, 'exchange_rates', { USD: 1 });
  });
  ...
  const [lastSync, setLastSync] = useState<Date | null>(() => {
    const saved = StorageService.getItem<string | null>(user.id, 'last_sync', null);
  ...
  useEffect(() => {
    ...
    const savedDateStr = StorageService.getItem<string | null>(user.id, 'last_active_month_year', null);
    ...
    StorageService.setItem(user.id, 'last_active_month_year', currentDateStr);
  }, [user.id]);

  useEffect(() => {
    StorageService.setItem(user.id, 'expenses', expenses);
  }, [expenses, user.id]);

  useEffect(() => {
    StorageService.setItem(user.id, 'settings', settings);
  }, [settings, user.id]);

  useEffect(() => {
    StorageService.setItem(user.id, 'exchange_rates', rates);
  }, [rates, user.id]);

  useEffect(() => {
    if (lastSync) StorageService.setItem(user.id, 'last_sync', lastSync.toISOString());
  }, [lastSync, user.id]);
      ...
      <div className="w-full px-2 pb-20 md:px-10 lg:px-16 py-3 md:py-8">
        ...
        <header className="flex items-center justify-between mb-3 md:mb-8 px-1">
          ...
          <div className="flex items-center space-x-1 bg-brand-card px-2 py-1 md:px-4 md:py-2 rounded-xl border border-white/5 shadow-lg group">
            ...
            <button
              onClick={logout}
              className="p-1 hover:bg-white/5 rounded-lg transition-colors border-l border-white/10 ml-1 pl-2"
              title="Sign out"
            >
              <LogOut className="w-3 md:w-3.5 h-3 md:h-3.5 text-brand-muted hover:text-red-400" />
            </button>
          </div>
        </header>
